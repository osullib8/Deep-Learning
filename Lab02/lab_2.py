...
def cross_entropy(w, X, y):
    ...
    c_entropy = y * np.log(p[0] + eps) + (1-y)*np.log(1-p[0] + eps)
    summing = -np.sum(c_entropy)
    ans = summing/n #Average of sum 
...
def gradient(w, X, y):
    ...
    p = predict(w, X) - y           # <replace '[0]' with a call to the predict function>
    grad = np.dot(p, X) #[0]       # use 'np.dot' to compute the vector
